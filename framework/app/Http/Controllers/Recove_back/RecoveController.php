...
class RecoveController extends Controller
{
	...
	public function store()
	{
        ...
        foreach ($bitacora as $bit) 
		{
			...
			if($bit->status == 0)
				Pedimento::consultaPedimento($bit->id);
				
			if($bit->status == 1)
			{
				//$partidas = explode('>', $bit->partidas);
				for($i=1; $i <= $bit->tpartidas; $i++)
					Pedimento::consultaPartida($i,$bit->id);	
        ...
        { 
        	...
        	$getReferen = BitacoraPedimento::find($bcove->bitacora_pedimento_id);
        	$referen = $getReferen->aduana.'-'.$getReferen->patente.'-'.$getReferen->pedimento;
        	$data_cove = COVE::consultaCOVE($referen, $bcove->cove);
        	BitacoraCove::where('id',$bcove->id)->update(['status' => $data_cove['status'],'observaciones' => $data_cove['observa']]);
        }
		$bitacora_new = BitacoraPedimento::where('status',0)->where('bitacora',0)->paginate(10);		
		if(count($bitacora_new) > 0)
			$type = 0;
	...
	{
		$path = dirname(dirname(dirname(dirname(dirname(dirname(__FILE__)))))).'/public_html/clientes/ftp/ihb/pdf/2014/'; 

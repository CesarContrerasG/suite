...
class ItemPrueba extends Model{
    ...
    public static function generarXML($item,$aduana,$patente,$pedimento,$opera, $id_company)
    {
        $company = Company::find($id_company);
        $referen = $aduana.'-'.$patente.'-'.$pedimento;
        $path_xml = storage_path() . '/xml/pedimento';
        $path = $path_xml .'/'. $company->name.'/';
    ...
    public static function fillData($response, $referen, $company)
    {
                ...
                foreach($xml->children($namespaces['S'])->Body as $body) 
                {
                    ...
                    {   
                        if($content->children()->tieneError == 'false')
                        {
                            ...
                            $dataIt = ItemPrueba::getFields($itemChild,$referen);
                            if(!is_null($dataIt))
                                ItemPrueba::insert($dataIt);                                                                       
                            ...
                            {
                                $dataIItem = IdentifierItemPrueba::getFields($idxp, $dataIt['pk_sec'], $referen, $company);
                                if(!is_null($dataIItem))
                                    IdentifierItemPrueba::insert($dataIItem);
                            }
                        }
                    }
                }

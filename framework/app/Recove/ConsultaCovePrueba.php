...
use App\Cove\Detail;
use App\Cove\CovePrueba as Cove;
use App\Cove\Seal;
use App\Recove\PedimentoPrueba;
use App\Recove\PathDownload;

class ConsultaCovePrueba extends Cove{

    public static function searchCove($cove, $referen, $id_company)
    {
        $seal = Seal::first();
        ...
        if(count($exist_cove) == 0)
        {
            ...
            if(!file_exists($path))
                @mkdir($path);
            $response = ConsultaCovePrueba::responseXML($seal, $cove, $path_plantilla, $path); 
            if(file_exists($response))
            {
                $id_cove = ConsultaCovePrueba::insertCove($response, $seal, $referen);
                if($id_cove != '')
                {
                    ...
                    if(!is_null($seal))
                    {
                        ...
                        $xml_cadena = Cove::createXML($seal, $cove, 1, $automotriz, $path_plantilla, $company->id); 
                        ...
                        {                            
                            ...
                            $pedimento = PedimentoPrueba::selectRaw('YEAR(ref_fechapago) as periodo')->where('pk_referencia', $referen)->first();
    ...
    public static function insertCove($response, $seals, $referen)
    {
        ...
        $tipoped = PedimentoPrueba::where('pk_referencia', $referen)->first();     
        $id_cove = '';   
        if(is_null($tipoped))
        {
            $pk_tipo = 1; 

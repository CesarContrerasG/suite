...
class Pedimento extends Model{
    ...
    //======================== LISTADO DE BUSQUEDA =====================================
    public static function searchList($aduana, $date, $start, $rfc)
    {         
        ...
        $response = Pedimento::requestPedimentoXML($rfc, $aduana, $date, $path);         
        if(file_exists($response))
            Pedimento::readList($response, $aduana, $date);     
    ...
    public static function requestPedimentoXML($rfc, $aduana, $date, $path)
    {
        ...
        Pedimento::setParams($aduana, $rfc, $date, $xml);
        $xml->saveXML($file);
        if(file_exists($file))
        {  
    ...
    public static function readList($response, $aduana, $date)
    {
    ...
    //////////////////////////////////////////////////////////////////////////////////
    public static function requestCompleteXML($bitacora)
    {
        $company = auth()->user()->company_name;              
        $path_xml = storage_path() . '/xml/pedimento';
        $path = $path_xml.'/'. $company. '/';  
        $optr01 = Pedimento::where('pk_aduana', $bitacora->aduana)->where('pk_patente', $bitacora->patente)->where('pk_pedimento', $bitacora->pedimento)->first();
        $referen = $bitacora->aduana.'-'.$bitacora->patente.'-'.$bitacora->pedimento;
        //================================ CONSULTA PEDIMETO ========================================== 
        //$response = Pedimento::requestCompleteXML($bitacora->aduana, $bitacora->patente, $bitacora->pedimento, $path);  
        ...
        Pedimento::setUserToken($xml);
        Pedimento::setComplete($bitacora->aduana, $bitacora->patente, $bitacora->pedimento, $xml);
        $xml->saveXML($file);
        if(file_exists($file))
        {  
    ...
    public static function fillData($response,$aduana,$patente,$pedimento, $idPedimento)
    {
        $xml = simplexml_load_file($response);
        $referen = $aduana .'-'. $patente.'-'.$pedimento;
        $namespaces = $xml->getNamespaces(true);
            ...
            foreach($xml->children($namespaces['S'])->Body as $body)
            {
                foreach($body->children($namespaces['ns2'])->consultarPedimentoCompletoRespuesta as $content)
                {
                    if($content->children($namespaces['ns3'])->tieneError == 'false')
                    {
                        ...
                        {
                            $dataH = Pedimento::getFields($content,$aduana,$patente,$pedimento);        
                            $exist_optr01 = Pedimento::where('pk_aduana', $aduana)->where('pk_patente', $patente)->where('pk_pedimento',$pedimento)->count();
                            if($exist_optr01 == 0)
                            {
                                Pedimento::insert($dataH);            
                                ...
                                foreach ($identif as $ide)
                                {
                                    ...
                                    {
                                        $dataId = Identifier::getFields($key,$referen);
                                        if(!is_null($dataId))
                            ...
                            {
                                Pedimento::getDataExist($referen, $idPedimento);
                            }                             
                            ...
                            if($toptr03 < $total_item)
                            {
                                foreach($items as $item)         
                                    Pedimento::consultaPartida($item, $idPedimento, $aduana, $patente, $pedimento, $noOperacion);
                            }
                            ...
                            Pedimento::where('pk_referencia', $referen)->update(['ref_efectivo' => $cash, 'ref_otros' => $other, 'ref_total' => $total_cont]);
                            //======================================================================================
        ...
        BitacoraPedimento::where('id', $idPedimento)->update(['observaciones' => $observaciones,'status' => $status]);
    ...
    public static function getFields($content,$aduana,$patente,$pedimento)
    {
        ...
        if(isset($content->pedimento->rectificacion)) 
        {
            ...
            $clave = 'R1';
            Pedimento::where('pk_referencia', $referencia_original)->update(['ref_referen' => $referen, 'ref_status' => 'R']);
        } 
        else
        {
            $referencia_original = '';
    ...
    public static function consultaPartida($item, $idPedimento, $aduana, $patente, $pedimento, $noOperacion)
    {
        $referen = $aduana.'-'.$patente.'-'.$pedimento;       
        $search_optr03 = Item::where('pk_referencia', $referen)->where('pk_sec', $item)->count();
        if($search_optr03 == 0)
        {
            $response = Item::generarXML($item, $aduana, $patente, $pedimento, $noOperacion);  
            if(file_exists($response))
                Item::fillData($response, $aduana.'-'.$patente.'-'.$pedimento);
        }
  
    }

    public static function getDataExist($referen, $idPedimento)
    {

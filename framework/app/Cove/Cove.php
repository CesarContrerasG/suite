...
class Cove extends Model{
    ...
    public static function createXML($seal, $cove, $type, $automotriz, $path, $company)
    {         
        ...
        {            
            ...
            // Encabezado 
            Cove::encabezado($cove, $seal, $tag_encabezado, $ns['oxml'],  $ns['prefix_oxml']);
            // Factura     
            ...
            {
                if($automotriz != 0)
                   Cove::emisorAndDestinatario($comprobante, $invoice, $ns['oxml'], $ns['prefix_oxml']);
                
                Cove::factura($cove, $invoice, $id, $cove->cove_relacion, $comprobante, $ns['prefix_oxml'], $ns['oxml'], $automotriz);  
                if($cove->cove_relacion == 0)
                {
                    if($automotriz == 0)
                        Cove::emisorAndDestinatario($comprobante, $invoice, $ns['oxml'], $ns['prefix_oxml']);
                    foreach($cove->inventory()->get() as $idinv => $inventory )
                        Cove::mercancias($comprobante, $inventory, $ns['oxml'], $ns['prefix_oxml'], $idinv);
                }
            }
        }
    ...
    public static function encabezado($cove, $seal, $tag_encabezado, $oxml, $prefix)
    {
    ...
    public static function factura($cove, $invoice, $id, $relacion, $comprobante, $prefix, $oxml, $automotriz)
    {
        ...
        if($relacion == 1)
        {
            ...
            if($automotriz == 0)
                Cove::emisorAndDestinatario($tag_facturas, $invoice, $oxml, $prefix);                
                       
            foreach($cove->inventory()->where('inv_factura', $invoice->inv_factura)->get() as $idinv => $inventory)
            {
                Cove::mercancias($tag_facturas, $inventory, $oxml, $prefix, $idinv);
            } 
        }       
    }
    ...
    public static function createAcuse($id, $seal, $company, $referen)
    {
        ...
        $pedimento = Pedimento::selectRaw('YEAR(ref_fechapago) as periodo')->where('pk_referencia', $data_cove->pk_referencia)->first();
